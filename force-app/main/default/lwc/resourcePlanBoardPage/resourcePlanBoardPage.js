// force-app/main/default/lwc/resourcePlanBoardPage/resourcePlanBoardPage.js
...
export default class ResourcePlanBoardPage extends NavigationMixin(LightningElement) {
    ...
    _buildLane(fte, planDate = null) {
        ...
        const contrOffset = this._assignments
            ...
            .reduce((s, a) => {
                ...
                return s + (c?.availableHours || a.hoursPerWeek || 0);
            }, 0);
    ...
    get boardCls()         { return `plan-board plan-board--inline${this._whatIfMode ? ' plan-board--whatif' : ''}`; }
        ...
        return team.map((lane, i) => {
            ...
            const cards = lane.cards.map((card, ci) => {
                ...
                const projContractors = this._assignments
                    ...
                    .map(a => {
                        const c = (this._raw.contractorPool || []).find(x => x.id === a.contractorId);
                        return c ? { ...c, assignmentId: a.id, hoursPerWeek: c.availableHours || 0 } : null;
                    }).filter(Boolean);
